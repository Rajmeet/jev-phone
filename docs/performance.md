...
Three goals in two first-party apps is a smoke test, not an evaluation. Nothing here scrolls a long list, handles a picker or a permission prompt, or leaves the app it started in except by `OPEN_APP` from the list. The [iOSWorld](https://github.com/ljang0/iOSWorld) run with the same policy inside phone-use's harness is reported separately; on that benchmark most tasks require a reported answer, which a System One model cannot produce.
