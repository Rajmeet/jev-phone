...
**iOS and Android.** Bold Text on in 11.2 s and 4 decisions on an iPhone simulator; a new contact typed and saved in 19.4 s there, 51.5 s on a local Android emulator and 54.8 s on a cloud Android phone — 6 decisions each. All verified by reading the phone afterwards (the Android ones through adb), not by trusting the model.
...
Seven verified runs across iOS and Android: **Bold Text on in 11.2 s / 4 decisions** from the Settings root, **General → About in 14.9 s / 5 decisions** from two screens deep in another section, and **a contact created in 19.4 s / 6 decisions** with two typed values (text helper 339 and 406 ms) — all on a local iPhone simulator. On **Android** (a local emulator over adb, and a cloud phone): Airplane mode on in 29.2 s / 4 decisions locally (`adb shell settings get global airplane_mode_on` read 0 before and 1 after) and 13.0 s / 2 on the cloud phone; the **same contact goal in 51.5 s locally and 54.8 s on the cloud phone, 6 decisions each** (OPEN_APP Contacts → Create contact → two TYPEs → Save), the row present in the contacts database afterwards. Android costs ~10 s per device step against ~3 s on the iOS simulator; the decisions are the same. Jev latency was 260–550 ms per decision, with two outliers at 1.1–1.4 s on the busiest screens. Full records, and the five discarded attempts with what each one changed, are in [performance.md](docs/performance.md) and [measurement.json](docs/measurement.json).
